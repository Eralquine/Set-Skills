...
agent-reach install --env=auto        # read-only check by default — lists what's missing
# only after the user explicitly approves system changes:
agent-reach install --env=auto --system
```
Never pass `--system` on your own initiative — it installs/configures system-level
tools (yt-dlp, gh, browser CLIs, etc.); get explicit approval first, same as any
other system-modifying install. Never use `sudo` unless the user explicitly approved it.
Full install guide (fetch if a specific channel needs setup):
https://raw.githubusercontent.com/Panniantong/agent-reach/main/docs/install.md

Login-backed platforms (Twitter/X, Reddit, XiaoHongShu, Facebook, Instagram)
need the **user's own** cookies/tokens — this skill never logs the user in or
reads their browser cookies automatically. If no session/cookie is already
provided, ask the user for one (or a manual Cookie-Editor export) rather than
attempting to establish one yourself.
